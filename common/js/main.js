...
CommonUI.initSubtitleContext = function() {
...
CommonUI.initSettingContext = function() {
...
CommonUI.initVariable = function() {
  ...
  // menu context
  this.initSubtitleContext();
  this.initSettingContext();
};
...
CommonUI.initUI = function() {
  ...
  this.vopLogo = document.querySelector('.vop-logo');
...
CommonUI.initUIEventListeners = function() {
  this.vopPlayer.setAttribute('tabindex', -1);
  ...
  this.vopLogo.onclick = this.onLogoClick.bind(this);
...
CommonUI.resizePlayer = function() {
  ...
  console.log('resize player w: ', dstWidth, 'h: ', dstHeight);
...
CommonUI.resetUI = function() {
  ...
  // restore ui if ad is playing
  this.vopProgressBar.style.pointerEvents = 'auto';
  this.vopScrubberContainer.style.display = 'block';
  this.vopSubtitlesBtn.style.display = 'none';
  // button's default display is 'inline-block', see https://www.w3.org/TR/CSS21/sample.html
...
CommonUI.enterFullscreen = function() {
  printLog('commonui enter fullscreen', LOG_DEBUG);
  var v = document.querySelector('.player');
...
CommonUI.leaveFullscreen = function() {
  printLog('commonui leave fullscreen', LOG_DEBUG);
...
CommonUI.onPause = function() {
  var currPaused = this.player_.isPaused();
  if (this.isTrickPlay()) {
    this.cancelTrickPlay();
  } else if (!currPaused) {
    this.player_.pause();
    this.updatePlayBtnUI(true, false);
    this.vopSpinner.style.display = 'none';
  }
};

CommonUI.onForwardButtonClick = function() {
  console.log("trick mode support? " + this.player_.isSupportTrickPlay());
  this.trickPlayRate = (this.trickPlayRate < 0 || this.trickPlayRate > 8) ? 1 : this.trickPlayRate * 2;
  this.player_.setTrickPlay(this.trickPlayRate);
};

CommonUI.onRewindButtonClick = function() {
  this.trickPlayRate = (this.trickPlayRate > 0 || this.trickPlayRate < -8) ? -1 : this.trickPlayRate * 2;
  this.player_.setTrickPlay(this.trickPlayRate);
};

CommonUI.cancelTrickPlay = function() {
  this.player_.cancelTrickPlay();
  this.trickPlayRate = 1;
};

CommonUI.isTrickPlay = function() {
  return this.trickPlayRate !== 1;
};
...
CommonUI.onPlayerMouseleave = function() {
  ...
  // check whether subtitle/setting menu is poped
  var poped = false;
  if (this.settingMenuContext.currMenu !== 'none' || this.subtitlesMenuContext.currMenu !== 'none') {
    poped = true;
  }
  var paused = this.player_.isPaused();
  if (!poped && !paused && !this.progressBarContext.mousedown && !this.flagVolumeSliderMousedown) {
...
CommonUI.onPlayerClick = function(e) {
  if (!this.flagPlayerInited || this.noPlayerClick) {
    this.noPlayerClick = false;
    return;
  }
...
CommonUI.onProgressBarMousedown = function(e) {
  printLog('progress bar mousedown', LOG_DEBUG);
  e.preventDefault();
  e.stopPropagation();
...
CommonUI.onProgressBarMouseleave = function() {
  printLog('progress bar mouseleave', LOG_DEBUG);
  this.vopTooltip.style.display = 'none';
};
...
CommonUI.docProgressBarMousemove = function(e) {
  printLog('document progress bar mousemove', LOG_DEBUG);
...
CommonUI.docProgressBarMouseup = function(e) {
  printLog('document progress bar mouseup', LOG_DEBUG);
  ...
  if (this.progressBarContext.posBeforeMousedown != this.progressBarContext.movePos) {
    ...
    // meed to consider the seekable range when seek a dvr stream.
    var seekPos = this.progressBarContext.movePos;
    var streamInfo = this.getStreamInfo();
    if (streamInfo.type === 'dvr') {
      var offset = streamInfo.duration - this.progressBarContext.movePos;
      seekPos = streamInfo.seekableRange.end - offset;
    }
    this.player_.setPosition(seekPos);
  }
...
CommonUI.onPopupMenuBlur = function(e) {
  printLog('popup menu blur', LOG_DEBUG);
  if (this.browser === 'IE') {
...
CommonUI.onLogoClick = function(e) {
  e.stopPropagation();
};
...
CommonUI.onVolumeSliderMousedown = function(e) {
  ...
  printLog('volume slider mousedown', LOG_DEBUG);
...
CommonUI.onSettingClick = function() {
  ...
  printLog('setting click, currMenu: ' + this.settingMenuContext.currMenu, LOG_DEBUG);
...
CommonUI.onOpenFinished = function() {
  ...
  if (audioTracks && audioTracks.length > 0) {
    ...
    audioTracks.forEach(function(track) {
      var lang = '';
      if (track.bandwidth) {
        lang = Math.round(track.bandwidth / 1000) + ' kbps';
        if (track.lang && (track.lang !== '')) {
          lang += '(' + track.lang + ')';
        }
      } else {
        if (track.lang && (track.lang !== '')) {
...
CommonUI.onPlayPaused = function() {
  printLog('play paused', LOG_DEBUG);
  var paused = true;
  var ended = this.player_.isEnded();
  this.updatePlayBtnUI(paused, ended);
};
...
CommonUI.onPlayWaiting = function() {
  printLog('play waiting', LOG_DEBUG);
  this.vopSpinner.style.display = 'block';
};

CommonUI.onPlayPlaying = function() {
  printLog('play playing', LOG_DEBUG);
  this.vopSpinner.style.display = 'none';
};

CommonUI.onSeekStart = function() {
  printLog('seek start', LOG_DEBUG);
  this.vopSpinner.style.display = 'block';
};

CommonUI.onSeekComplete = function() {
  var position = this.player_.getPosition();
  printLog('seek complete, pos: ' + position, LOG_DEBUG);
...
CommonUI.onBitrateChanged = function(e) {
  if (e.mediaType === 'audio') {
    ...
    printLog('bitrate changed, video: ' + this.settingMenuContext.currQualityId, LOG_DEBUG);
    this.updateQualityMenuUI();
  }
};

CommonUI.onProgramChanged = function(e) {
  printLog('program changed', LOG_DEBUG);
...
CommonUI.getStreamInfo = function() {
  ...
  ret.seekableRange = seekableRange;
  return ret;
};
...
CommonUI.doCreateSettingsMenu = function() {
  ...
  // current quality, current audio track, etc.
  var qualityCnt = this.settingMenuContext.qualityList.length - 1;

  // created quality menu item
...
CommonUI.updateQualityMenuUI = function() {
  printLog('update quality menu ui', LOG_DEBUG);
...
CommonUI.updateAudioTrackMenuUI = function() {
  printLog('update audiotrack menu ui', LOG_DEBUG);
...
CommonUI.onQualityMenuClick = function(e) {
  printLog('quality menu click: ' + e.target.innerText, LOG_DEBUG);
...
CommonUI.onAudioTrackMenuClick = function(e) {
  printLog('audio track menu click', LOG_DEBUG);
...
CommonUI.onMainMenuBlur = function(e) {
  printLog('+main menu blur, currMenu: ' + this.settingMenuContext.currMenu, LOG_DEBUG);
  ...
  printLog('-main menu blur', LOG_DEBUG);
};

CommonUI.onSubtitleItemClick = function(e) {
  printLog('subtitle item click', LOG_DEBUG);
...
CommonUI.onSubtitleItemBlur = function(e) {
  printLog('subtitle item blur', LOG_DEBUG);
...
CommonUI.onSubtitlesBack = function(e) {
  printLog('subtitles back', LOG_DEBUG);
};

CommonUI.onQualityBack = function(e) {
  printLog('quality back', LOG_DEBUG);
...
CommonUI.onQualityMenuItemClick = function(e) {
  printLog('quality menuitem click, id: ' + e.currentTarget.dataset.id, LOG_DEBUG);
...
CommonUI.onQualityMenuItemBlur = function(e) {
  printLog('quality menuitem blur', LOG_DEBUG);
...
CommonUI.onAudioTrackBack = function(e) {
  printLog('audio track back', LOG_DEBUG);
  this.destroySettingsMenu();
  this.createSettingsMenu();
};
...
CommonUI.onAudioTrackItemBlur = function(e) {
  printLog('audio track item blur', LOG_DEBUG);
